package com.example.actividad1.services

import com.example.actividad1.domain.Dibujo

/**
 * Clase que contiene los datos que se les pasa al RecyclerView
 */
class DibujosProvider {

    companion object {
        // Lista mutable que almacenará objetos de tipo 'Dibujo'
        val dibujosList = mutableListOf<Dibujo>(
            // Añadiendo objetos 'Dibujo' a la lista con sus propiedades name, serie y photo
            Dibujo( "Goku",
                "Dragon ball",
                "https://depor.com/resizer/dV5TqPnaHmZP6yMI_GfeKL28T6I=/1200x1200/smart/filters:format(jpeg):quality(75)/cloudfront-us-east-1.images.arcpublishing.com/elcomercio/OCJ6ZQ4G4RHMRGDRYR7UBZQ6PY.jpg"
            ),
            Dibujo( "Homer Simpson",
                "The Simpson",
                "https://files.lafm.com.co/assets/public/styles/img_node_706x392/public/2019-05/homero_2_0.jpg.webp?VersionId=PZF0BqJIb2.wbcPfM2EgERCBZbhCucWD&itok=S77Ubu8f"
            ),
            Dibujo( "número 1",
                "código KND",
                "https://static.wikia.nocookie.net/knd/images/4/4d/Numbuh_1.jpg/revision/latest/scale-to-width-down/1200?cb=20221225185818"
            ),
            Dibujo( "hiroto kiyama",
                "Inazuma Eleven",
                "https://images2.wikia.nocookie.net/__cb20110620144261/inazuma-eleven/images/e/ee/Hiroto_Kiyama_'Gran'.jpg"
            ),
            Dibujo( "Ben",
                "Ben 10",
                "https://occ-0-2794-2219.1.nflxso.net/dnm/api/v6/E8vDc_W8CLv7-yMQu8KMEC7Rrr8/AAAABYJjYUCmXJvmLkirQQTEbDpZTyKVaWPm_xkAmi2k-WNXgbOSUytlt6FRFqKEjIjysLJvMAu1OgvzixtBvzoXXkd3scZmXQsnfPlW.jpg?r=332"
            ),
            Dibujo( "Agallas",
                "Agallas el perro cobarde",
                "https://es.web.img3.acsta.net/pictures/14/06/05/13/56/127570.jpg"
            ),
            Dibujo( "Kim",
                "Kim possible",
                "https://static.wikia.nocookie.net/disney/images/a/ab/Kim.gif/revision/latest?cb=20090424152434&path-prefix=es"
            ),
            Dibujo( "hustler kid",
                "La banda del patio",
                "https://i.pinimg.com/originals/15/58/23/1558234467c251f0fe1d16a2c20c15f4.jpg"
            )



        )

        // Función para agregar un nuevo 'Dibujo' a la lista 'dibujosList'
        /**
         *  Función para agregar un nuevo 'Dibujo' a la lista 'dibujosList'
         *  @param dibujo: clase dibujo que se va a añadir
         */
        fun addDibujo(dibujo: Dibujo) {
            dibujosList.add(dibujo)
        }
    }
}